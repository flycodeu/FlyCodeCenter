---
title: 视频智能分析平台（二）：录像存储与检索
createTime: '2026/03/01 19:23:46'
code: b3t6f601o
permalink: /blog/b3t6f601o/
tags:
  - MediaMTX
  - FFmpeg
  - WebSocket
...
本篇介绍录像存储与检索。代码用于说明模块协作，包含待接入的项目接口和配置，不能据此认定整个平台已经通过生产验证。
...
拼接播放列表时需区分 MPEG-TS 与 fMP4：fMP4 需要正确的初始化段引用；不能把任意独立 MP4 文件名连在一起就认定为可播放 HLS。
...
- 完全包含
- 部分重叠
- 端点相等视为不重叠，符合半开区间 `[start, end)` 的约定
