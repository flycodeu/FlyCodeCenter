...
| 特性 | 说明 |
|------|------|
| 项目接入 | 需接入查询、认证、存储和任务状态管理 |
| 📊 **进度追踪** | 实时显示导出进度和处理数量 |
| 🎯 **泛型设计** | 支持任意数据类型的导出 |
| 🖼️ **图片支持** | 内置图片异步下载和超时控制 |
| 💾 **分批处理** | 限制单批数据量；图片、样式和临时文件仍会占用资源 |
| 🔄 **断点续传** | 记录失败状态；重试策略需结合输出文件状态实现 |
...
### Q4: 只记录 offset 能实现断点续传吗？

不能。还需要稳定的数据快照、排序、已写入文件的恢复方式和原子进度记录。SXSSF 不能凭一个 offset 自动恢复已关闭的工作簿；若重新创建工作簿却跳过前面数据，会导出不完整文件。未实现文件恢复时应从头重试。下面只是记录查询位置：
```java
// 记录最后处理的offset
task.setLastOffset(offset);
exportTaskService.updateById(task);

// 仅作数据查询进度参考；不能据此直接续写已有 XLSX
