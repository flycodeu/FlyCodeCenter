...
- `onopen`：连接建立时触发。
- `onerror`：连接出错或中断时触发。
- `close()`：主动关闭连接并停止自动重连。原生 `EventSource` 没有 `onclose` 事件。
...
同一事件的字段逐行书写，空行结束一个事件块；不要在每个字段之间插入空行。流使用 UTF-8，支持 LF、CRLF 或 CR 行结束符。常见字段如下：

| 字段    | 含义                                                     |
| ------- | -------------------------------------------------------- |
| `data`  | 消息数据，可写多行；只有注释或控制字段的块可以不含 data。                   |
...
| 功能         | SSE                     | WebSocket                  |
| ------------ | ----------------------- | -------------------------- |
| 协议         | HTTP                    | 标准 WebSocket 协议（RFC 6455） |
| 通信方向     | 单向（服务器 → 客户端） | 双向                       |
| 数据格式     | 文本                    | 文本或二进制               |
| 是否自动重连 | 是                      | 否（需自行实现）           |
| 浏览器兼容性 | 较好（不支持 IE）       | 良好（现代浏览器均支持）   |
| 实现复杂度   | 简单                    | 复杂                       |
| 性能开销 | 取决于连接数、消息大小和实现 | 取决于连接数、消息大小和实现 |
...
- **代理缓冲与压缩**：先关闭会积攒消息的代理缓冲；启用压缩前验证每条消息能及时 flush。
- **断线恢复**：浏览器可发送 `Last-Event-ID`，但服务端还需保存可重放事件并实现恢复逻辑，单独设置 `id` 不会自动补发。
- **安全措施**：确保接口权限控制，避免被滥用。
...
事件格式及重连行为以 [WHATWG SSE 标准](https://html.spec.whatwg.org/multipage/server-sent-events.html) 为准。
