...
安装包携带的语言模型取决于发行版和安装选项。先用 `tesseract --list-langs` 查看已安装语言；缺少中文时，再安装对应语言包或下载 `.traineddata` 文件，并确认 `tessdata` 路径。参见 [Tesseract 安装说明](https://tesseract-ocr.github.io/tessdoc/Installation.html)。
