---
title: 视频智能分析平台（四）：运维与容量规划
createTime: '2026/03/01 19:23:46'
code: b1c5ha70m
permalink: /blog/b1c5ha70m/
tags:
  - MediaMTX
  - FFmpeg
  - WebSocket
...
本篇介绍运维与容量规划。代码用于说明模块协作，包含待接入的项目接口和配置，不能据此认定整个平台已经通过生产验证。
...
容量评估需要记录硬件、软件版本、码率、编码格式、转码策略和测试时长。仅给出“路数”或 GPU 型号无法复现结果。

| 环节 | 记录的输入条件 | 需要采集的结果 |
| --- | --- | --- |
| 设备接入 | 设备型号、固件、并发数、超时和重试策略 | 成功率、P50/P95/P99、错误分类 |
| 录像写入 | 路数、实际码率、切片周期、磁盘类型 | 写入吞吐、I/O 等待、文件关闭与索引延迟 |
| 模型推理 | 模型、输入尺寸、精度、批量大小、抽帧率 | 单帧延迟、吞吐、排队时间、显存 |
| 告警处理 | 输入事件速率、去重策略、数据库配置 | 积压数量、入库和推送延迟、丢失与重复 |

### 22.1 先估算录像带宽

忽略封装和文件系统开销时，持续写入量可用总码率估算：

```text
写入量（MB/s）≈ 路数 × 单路码率（Mb/s）÷ 8
```

例如 500 路、每路 4 Mb/s，理论媒体数据约为 250 MB/s。这是容量估算，不是磁盘压测成绩；实际还要考虑码率波动、回放读取和索引写入。

### 22.2 再测量完整链路

逐步增加接入和读取并发，同时记录错误率与延迟分位数。只有在采集到 CPU、磁盘、网络、模型队列等证据后，才能判断瓶颈所在。

保存测试脚本、参数、原始指标和日志，比较调整前后的同一组负载。仅凭线程数、分片时长或一次成功播放，不能推算整体容量。
