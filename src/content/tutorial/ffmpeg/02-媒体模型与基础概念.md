...
一个媒体文件可以包含视频、声音和字幕。先用容器、编码和 Stream 区分它们，再对照 `ffprobe` 的字段查看自己的文件。

如果还不清楚一帧是什么，或想先理解“视频怎样组成、前后画面怎样预测”，可以从[视频基础一](/tutorials/tffmpeg-bitstream/)开始，再回来查看文件属性。

## 容器、编码和 Stream

下面是假设有一条视频、一条音频和一条字幕的 MP4 结构，用于说明层级；实际轨道以探测结果为准。
...
`.mp4` 是常见容器扩展名，不能据此判断视频编码。同一段 H.264 可以放进 MP4 或 MKV。换容器需要重新封装；给文件改名只改变路径。
...
FFmpeg 日志里的 `Stream #0:1` 表示“第 0 个输入中索引为 1 的流”。这个索引覆盖该输入的所有流类型，但不跨输入文件。后面写 `-map` 时常用：
...
Packet、Frame、time base 和关键帧都很重要，但它们是为切片、同步、Filter 和排错服务的。先能用 `ffprobe` 读出容器和 Stream，再按第 03 篇完成任务。

依据：[ffprobe 官方文档](https://ffmpeg.org/ffprobe.html)、[FFmpeg 媒体处理说明](https://ffmpeg.org/ffmpeg.html#Detailed-description)、[AVStream 中的帧率定义（n7.1.1）](https://github.com/FFmpeg/FFmpeg/blob/n7.1.1/libavformat/avformat.h)。
