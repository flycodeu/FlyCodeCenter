---
title: 第五步：理解参数、Filter 与质量
createTime: '2026/08/31 10:05:00'
code: t17xaopev
permalink: /tutorials/t17xaopev/
summary: 通过参数位置读懂命令，理解 Filter 为何需要转码，并学会基本的质量与码率控制。
...
命令变长以后，先找到每个 `-i` 和输出文件名，再判断选项作用于哪里。本篇接着说明 Filter、编码质量和容器选项怎样配合。
...
多数非全局选项作用于紧随其后的输入或输出文件，并在下一个文件处重新设置。先按文件划分：
...
| `-r` | 输出帧率相关处理 | 会影响丢帧/补帧，先理解 `fps` Filter 再使用 |

下面沿用教程中的 `libx264 -crf 23 -preset medium` 作为演示配置。CRF 控制质量与体积的取舍，preset 控制编码速度与压缩效率，实际效果需要用自己的素材比较：

```powershell
ffmpeg -i input.mp4 -c:v libx264 -crf 23 -preset medium -c:a aac -b:a 128k output.mp4
```

保持输入和其他参数相同时，对 `libx264` 降低 CRF 通常会提高质量并增大文件；提高 CRF 则相反。某个数值不能保证“接近原片”，也不能跨编码器直接比较。CRF 23 不是 23% 画质。
...
`-maxrate` 和 `-bufsize` 描述缓冲约束，不保证每一秒比特数完全相同。最终体积还受音频、容器和内容影响。`-fs` 达到文件大小限制后会停止继续写入，实际文件可能略大于该值；它不会自动调整质量来保留完整时长。
...
逗号表示从左到右：先 `scale`，再 `fps`。视频经过 Filter 后必须编码；没改的音频仍可复制。
...
这里的 `[outv]` 是带标签输出，必须映射一次且只能一次。无标签的复杂 Filtergraph 输出会自动加入第一个输出文件，即使该文件已写了其他 `-map`。显式命名并映射更便于检查连接关系，详见 [FFmpeg Filters](/tutorials/tffmpeg-filters/)。
...
依据：[FFmpeg 选项与命令结构](https://ffmpeg.org/ffmpeg.html#Options)、[Filter 官方文档](https://ffmpeg.org/ffmpeg-filters.html)、[libx264 编码选项](https://ffmpeg.org/ffmpeg-codecs.html#libx264_002c-libx264rgb)、[MP4 封装与 faststart](https://ffmpeg.org/ffmpeg-formats.html)。
