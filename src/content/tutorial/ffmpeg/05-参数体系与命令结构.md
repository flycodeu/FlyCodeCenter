...
命令变长以后，先找到每个 `-i` 和输出文件名，再判断选项作用于哪里。本篇接着说明滤镜、编码质量和容器选项怎样配合。
...
这里的 `[outv]` 是带标签输出，必须映射一次且只能一次。无标签的复杂滤镜输出会自动加入第一个输出文件，即使该文件已写了其他 `-map`。显式命名并映射更便于检查连接关系，详见 [FFmpeg Filters](/tutorials/tffmpeg-filters/)。
...
依据：[FFmpeg 选项与命令结构](https://ffmpeg.org/ffmpeg.html#Options)、[滤镜官方文档](https://ffmpeg.org/ffmpeg-filters.html)、[libx264 编码选项](https://ffmpeg.org/ffmpeg-codecs.html#libx264_002c-libx264rgb)、[MP4 封装与 faststart](https://ffmpeg.org/ffmpeg-formats.html)。
