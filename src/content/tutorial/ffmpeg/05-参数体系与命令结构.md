...
| `-vf` | 单输入 Video Filterchain | `-vf "scale=1280:-2"` |
...
| `-ar` | 输出采样率 | `-ar 48000` |
| `-ac` | 输出声道数 | `-ac 2` |
| `-af` | Audio Filterchain | `-af "volume=1.5"` |
| `-an` | 禁止输出音频 | 只生成无声视频 |
...
`[0:v]`、`[1:v]` 是 input Stream，`[logo]`、`[outv]` 是 Filtergraph 中的临时 Label，不是文件名。Label 输出必须通过 `-map` 放进输出。
...
这里的 `logo` 和 `outv` 只是 Filtergraph 内部的 Link Label；没有 `-map "[outv]"`，Filter 生成的结果不会自动进入输出文件。
