...
summary: 用可暂停、逐步播放的动画，看懂 I/P/B 帧怎样取得预测依据、传递差值、重建画面，以及解码与显示顺序。
...
**编码器在压缩一帧时，已经拿到了这帧原画面。** 它可以把原画面与已有参考逐块比较，寻找相似区域，记下取样位置，再把仍然不同的地方记成差值。解码器收到这些依据后，按相同规则计算。

比如小球只是从左边移到右边，大部分外观都能从参考画面取用；小球如果变亮，就再记录亮度差。这就是“预测”能够有用的原因：视频里存在可重复利用的图像信息，剩下的差别也有数据说明。

先保持 **P**，点击“下一步”：观察左边金框怎么找到参考、下方怎样取得预测画面、差值怎样把亮点补上。再切换 **I** 和 **B**，比较它们从哪里取得依据。取消“让当前小球变亮”，可以看到差值随原画面变化。

<video-coding-demo lesson="prediction" mode="P">
  <p>交互演示需要 JavaScript。文字版：I 使用本幅画面的邻近像素；P 可以使用已重建的其他画面；本例 B 组合两幅已重建参考。编码器先计算预测，再记录“原画面减预测”的差值；解码器计算“预测加差值”得到重建画面。</p>
</video-coding-demo>

动画里的移动表示参考区域的取样位置。它是一个未做量化的灰度像素实验，方便看清加减关系；真实 H.264/H.265 还包含分块选择、变换、量化等步骤。
...
| P 帧 | Predictive-coded picture | 像动画中的 P：可以从已经重建的其他画面取相似块，再结合差值重建当前块 |
| B 帧 | Bi-predictive-coded picture | 像动画中的 B：允许组合两个参考结果，再结合差值重建；也可以只使用单个参考 |

三种类型区别在于允许使用哪些预测方法，不是显示出来的画面完整程度不同。I/P/B 解码后都要得到一幅可显示的画面。P/B 也可以有帧内编码的块；规范中的类型细分到 Slice 等结构，这里使用常见的整帧称呼。
...
下面的动画将解码准备与显示拆开。点三次“下一步”，看 **P2 先于 B1 解码**；继续往下，才按 **I0、B1、P2** 的时间顺序显示。

<video-coding-demo lesson="frame-order">
  <p>交互演示需要 JavaScript。本例解码顺序为 I0、P2、B1：先解码两幅参考，再用 B1 携带的数据重建中间画面。显示顺序仍为 I0、B1、P2。B1 不是只靠两端图片自动插出来的。</p>
</video-coding-demo>
